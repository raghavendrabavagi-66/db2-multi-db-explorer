...
# ---------------------------------------------------------------------------
# Section 1 — Connection (left) + Database fleet (right)
# ---------------------------------------------------------------------------
with st.container(key="oe_section1"):
    setup_left, setup_right = st.columns([3, 7], gap="medium")
...
# ---------------------------------------------------------------------------
# Section 2 — Search criteria (left 30%) + Results (right 70%), fills viewport
# ---------------------------------------------------------------------------
with st.container(key="oe_section2"):
    search_left, results_right = st.columns([3, 7], gap="medium")
    ...
    with search_left:
        with st.container(border=True, key="oe_search_panel"):
            ...
            operator = st.radio("Match mode", MATCH_ORDER, horizontal=False, key="oe_match")
        ...
        with st.container(border=True, key="oe_results_panel"):
            ...
            if results is None:
                ...
                with st.container(key="oe_results_summary"):
                    ...
                    with toolbar_left:
                        show_only_matches = st.checkbox("Matches only", value=False, key="oe_matches_only")
                    with toolbar_right:
                        st.download_button(
                            "Download CSV",
                ...
                with st.container(key="oe_results_table"):
                    st.dataframe(view_df, width="stretch", hide_index=True, height="stretch")
